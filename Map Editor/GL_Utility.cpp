...
unsigned int GL_Utility::load_array_texture(const std::string* paths, unsigned int num_textures, bool filtering) {
	unsigned char** data = new unsigned char* [num_textures];
	...
	for (int i = 0; i < num_textures; i++) {
		...
		data[i] = stbi_load(paths[i].data(), &curr_w, &curr_h, &file_channels, 4);
	...
	unsigned int texture = create_array_texture(data, num_textures, w, h, filtering);
...
//		img_paths[i] = rel_path + img_paths[i];
//	}
//
//	unsigned int texture = load_array_texture(img_paths.data(), img_paths.size(), filtering);
//	
