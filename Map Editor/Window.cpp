...
void Window::init(int w, int h, std::string name, bool is_resizeable) {
	if (window != nullptr) {
		return;
	}
	...
	glfwWindowHint(GLFW_RESIZABLE, is_resizeable == true ? GLFW_TRUE : GLFW_FALSE);
...
void Window::clear_color(float r, float g, float b) {
...
bool Window::poll() { glfwGetWindowSize(window, &width, &height); glfwPollEvents();  return !glfwWindowShouldClose(window) && window != nullptr; }
